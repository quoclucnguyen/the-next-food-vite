...
export function GeminiApiKeySetup({
  ...
  const testApiKey = async () => {
    ...
    try {
      const client = new GeminiCoreClient(apiKey.trim());
      // If GeminiCoreClient does not have testConnection, you may need to implement or mock this method.
  ...
  const handleSave = async () => {
    if (!apiKey.trim()) {
      setTestResult({
        success: false,
        message: 'Please enter an API key to save.',
      });
      return;
    }
  ...
  const inputPlaceholder = hasApiKey
      ...
      <CardContent className='space-y-4'>
        ...
        {hasApiKey && (
          <Alert>
            ...
            <AlertDescription>
              Gemini API is configured and ready to use for recipe suggestions
        ...
        <Alert>
          <AlertDescription className='text-sm'>
            <strong>Security:</strong> Your API key is stored securely in the
            database and is only used for API calls. It is never shared with
            third parties and persists across browser sessions.
          </AlertDescription>
        </Alert>
      </CardContent>
    </Card>
