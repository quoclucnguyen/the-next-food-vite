...
export function RecipeSuggestions({ ingredients }: RecipeSuggestionsProps) {
  ...
  const generateSuggestions = async () => {
    ...
    try {
      ...
      if (!selectedModel) {
        setError(
          'No AI model selected. Please go to Settings and select a Gemini model to enable recipe suggestions.'
        );
        return;
      }

      // GeminiCoreClient does not implement generateRecipeSuggestions.
      setError(
        'GeminiCoreClient does not implement generateRecipeSuggestions.'
      );
      setSuggestions([]);
      ...
      <Card>
        <CardContent className='p-6 text-center'>
          ...
          <p className='text-sm text-gray-600 mb-4'>
            Configure your Gemini API key to get personalized recipe suggestions
        ...
        {suggestions.length === 0 && !loading && !error && (
          <div className='text-center py-4'>
            <p className='text-gray-600 mb-4'>
              Nhận gợi ý công thức cá nhân hóa dựa trên nguyên liệu có sẵn của
              bạn.
            </p>
            <Button
