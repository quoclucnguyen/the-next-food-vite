import {
  GeminiCosmeticService,
  type AIAnalyzedCosmeticItem,
} from '@/lib/gemini';
...
export function useCosmeticImageAnalysis({
  ...
  const [aiGeneratedInfo, setAiGeneratedInfo] =
    useState<AIAnalyzedCosmeticItem | null>(null);
  ...
  const handleImageReadyForAI = async (
    ...
    try {
      ...
      const geminiClient = new GeminiCosmeticService(
        settings.geminiApiKey,
        selectedModel
      );
      const result = await geminiClient.analyzeCosmeticImage(
        base64Image,
        mimeType
      );
